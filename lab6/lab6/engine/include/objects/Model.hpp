...
class Model : public ObjectBase {
  public:
    ...
    Model(string const &path, bool gamma = false)
        : ObjectBase("model"), gammaCorrection(gamma) {
    ...
    void draw(Shader &shader) const override {
        shader.use();
        shader.set("modelMatrix", ObjectBase::__placement);
        ...
        for (const auto &mesh : meshes) {
            for (const auto &vec : mesh.vertices) {
                glm::vec4 transformedVertex =
                    __placement * glm::vec4(vec.Position, 1.0f);
  ...
  private:
    ...
    void loadModel(string const &path) {
        {
            ...
            if (error != GL_NO_ERROR) {
                std::cout << "OpenGL error before: " << error << std::endl;
            }
        }
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene) {
        ...
        // 1. diffuse maps
        vector<MeshTexture> diffuseMaps = loadMaterialMeshTextures(
            material, aiTextureType_DIFFUSE, "texture_diffuse");
        textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
        // 2. specular maps
        vector<MeshTexture> specularMaps = loadMaterialMeshTextures(
            material, aiTextureType_SPECULAR, "texture_specular");
        textures.insert(textures.end(), specularMaps.begin(),
                        specularMaps.end());
        // 3. normal maps
        std::vector<MeshTexture> normalMaps = loadMaterialMeshTextures(
            material, aiTextureType_HEIGHT, "texture_normal");
        textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
        // 4. height maps
        std::vector<MeshTexture> heightMaps = loadMaterialMeshTextures(
    ...
    // they're not loaded yet. the required info is returned as a MeshTexture
    // struct.
    vector<MeshTexture> loadMaterialMeshTextures(aiMaterial *mat,
                                                 aiTextureType type,
                                                 string typeName) {
        vector<MeshTexture> textures;
...
inline unsigned int TextureFromFile(const char *path, const string &directory,
    ...
    if (img.getPixelsPtr()) {
        ...
        std::cout << "MeshTexture failed to load at path: " << path
                  << std::endl;
    }
